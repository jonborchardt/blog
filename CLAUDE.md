...
- Post URLs are flat: `/<slug>/`. Reserved route names live in `src/lib/reserved-slugs.ts` (`all-posts`, `series`, `about`, `admin`, `404`, `og`, `llms.txt`, `search-index.json`, `rss.xml`, `robots.txt`, `sitemap-index.xml`, `museum`); add there whenever a new top-level route is created.
...
- **No em dash (`—`) in anything a reader sees**: post prose, frontmatter, captions/alt/`summary` text, SVG label text, component copy. Reword with a colon, comma, parentheses, or a new sentence — never just delete. A YAML `description` that gains a colon must be double-quoted; a table "no data" cell is `n/a`. Exempt: historical records (`Details` transcript content, `public/museum/`) and internal `research/` briefs.
- Every post has a `hero` (1500×600 — 2.5:1, enforced by the schema — next to `index.mdx`, meaningful `alt`); it shows full-width on the post and listings crop it to the centre 60% (`aspect-card` + `object-cover`), so the key content must live in the centre. `npm run new-post` copies `src/assets/hero-placeholder.png` as `./hero.png` — replace it: unless an image was supplied, draw one via skill `create-hero` (`hero.svg` → `npm run render-hero -- <slug>`). Every series sets its own fresh `hero` in `src/config/series.ts` (`src/assets/series/<id>.png`, same contract, rendered via `npm run render-hero -- series/<id>`); never a post's hero — series pages fail the build without one.
...
- Colocated audio: short clips live in the post directory and are `import`ed, then played through `SoundBite` (never a raw `<audio controls>`, never a literal path string: the build cannot see it). Every play button's accessible name says what it plays; the dist checks enforce both.
...
- Site identity/nav/featured post: `src/config/site.ts`. Author: `src/config/author.ts`. Homepage intro prose lives in `src/pages/index.astro`; `/about/` is a static MDX page under `src/pages/`.
...
- Every post is also served as raw markdown at `/<slug>/index.md` (`src/pages/[slug]/index.md.ts`), and `/llms.txt` indexes published posts — both from `src/lib/llms.ts`; drafts are gated by `getPosts`.
