...
2. **Write `src/content/posts/<slug>/hero.svg`** by hand:
   ...
   - Fixed palette (it becomes a raster, so it cannot follow the theme): background `#f7f8fa`, ink `#2b3340`, primary `#2f6f9f`, accent `#dbe7f1`, highlight `#f0b35a`. Reuse these across posts so listings look like one site; invert (ink background) only when the subject is literally dark (night sky, terminal), and a series may pick its own highlight.
