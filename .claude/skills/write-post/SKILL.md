...
When `authority` is present, treat that repository as the source of truth for domain-specific and technical claims. Delegate domain investigation to an agent operating in that repository. That agent should inspect its repo-local `CLAUDE.md`, relevant code, docs, tests, data, and memory/context as available, and make domain decisions rather than merely returning files. Do not independently reinterpret implementation details when the authority agent can inspect them.

The blog repo owns the artifact: story, prose, MDX/React implementation, visual design, accessibility, and presentation. The authority repo agent owns factual correctness: behavior, terminology, data meaning, technical claims, caveats, and semantic validation of visuals.
...
2. Before writing domain claims, delegate investigation to the authority agent. Ask it for the relevant facts, implementation evidence, terminology, caveats, and important mental models/omissions for the intended post. Then write `src/content/posts/<slug>/index.mdx`:
...
3. **Visual pass — mandatory, no sign-off needed.** Re-read the finished prose and add every visual that earns its place; build each with `create-visual` (its ladder picks the medium — Mermaid → SVG → HTML/CSS → island; do not default to React). Do this even when the author only asked for text.
   ...
   - When the post describes code, check every diagram against the authority agent/source before drawing it — a plausible-looking wrong diagram is worse than none.
