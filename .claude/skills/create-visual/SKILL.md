...
- Visible text that must stay readable (a caveat, footnote, legend) is a sibling _after_ `VizFigure` (or `Figure`'s `caption`), never a child — non-interactive `VizFigure` hides all children.
