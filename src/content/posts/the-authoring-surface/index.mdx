...
hero:
  src: ./hero.png
  alt: Stacked authoring blocks (callout, tabs, code) with one island widget marked by a lightning bolt
draft: false
---
...
import BuildPipelineViz from "./build-pipeline-viz.svg";
import VisualLadderViz from "./visual-ladder-viz.svg";
...
them, doing its actual job; this page is its own demo. Nearly all of them ship no script beyond
a few inline lines; the post ends with the one real exception, a React island, and the case a
component has to make before it's allowed to be one: the reader genuinely needs to interact
with it.
...
A blog post is a document. Documents don't need a JavaScript framework to be read, and every
...
![A schematic card on a grid background, standing in for the screenshots real posts carry](./pipeline.png)
...
`Figure` also takes a `zoom` flag: it wraps a plain `<img>` in a button that opens a full-size
lightbox, for screenshots whose fine print matters.
...
When a list is an inventory of short items rather than an argument, `ListGrid` turns the tall
bullet column into a grid of boxes. It wraps a plain Markdown `-` list, so like `Steps` it
degrades to exactly that. Everything that lives in one post's directory:
...
`Steps` is an ordered list with a connector line. The Markdown stays a plain `1.` list, so it
...
Long, optional material folds away in a native `<details>` element, no script:
...
<Details summary="The full frontmatter contract">
  ...
  - `hero { src, alt }` (required, 1500×600) and optional `ogImage`
...
Three primitives carry media too heavy to load unasked, and each makes the reader opt in.
`Video` embeds a local file or a YouTube player, with a required accessible title. `Gallery`
steps through a set of screenshots one at a time with prev/next arrows and a full-size escape
hatch; [the TV data explorers post](/tv-data-explorers/) walks whole interfaces that way.
`MuseumEmbed` is the click-to-load frame around this site's preserved live demos, so a
multi-megabyte 2014 page costs nothing until the reader presses the button;
[Drag the Timeline, Refilter the Page](/drag-the-timeline-refilter-the-page/) is built around
one. None of them appear on this page — this post has no video, screenshot set, or museum demo.
That absence is the rule in action: a primitive shows up when its content does, never for
decoration.

## Diagrams and math

Diagrams are hand-drawn SVG files that live next to the post and render inline **at build
time**; the reader gets a picture, not a script that draws one. The house style is pictorial by
default: a rail line, a staircase, a magnifier, a drawing of the mechanism rather than a grid of
labeled boxes. Every diagram gets a sentence describing it and a name and summary on its figure,
because a picture without words is invisible to a screen reader. This one shows the path from an
MDX file to a deployed page:

<VizFigure
  name="From source to deployed page"
  summary="MDX source is validated, built to static HTML, tested, and deployed to GitHub Pages; a validation error sends the file back to be edited."
>
  <BuildPipelineViz class="mx-auto h-auto w-full max-w-2xl" aria-hidden="true" />
</VizFigure>

Because each drawing keeps its neutral strokes and text on `currentColor`, the same file follows
the reader's theme natively: no dark-mode filter, no second render.
...
There is no chart library on this site and no third-party widget behind any of the primitives
above — even the gallery and the museum embed are a few dozen lines of inline script, not a
dependency and a maintenance promise. When a post wants a visual,
the house rule is a ladder: stop at the first rung that holds.

<VizFigure
  name="The visual ladder"
  summary="Four rungs in order of preference: hand-drawn pictorial SVG, static SVG chart, HTML and CSS, React island. The first three cost zero script; only the island ships JavaScript, and only when the reader must manipulate the visual."
>
  <VisualLadderViz class="mx-auto h-auto w-full max-w-2xl" aria-hidden="true" />
</VizFigure>

A flow, a state machine, a mechanism: that's a hand-drawn picture. A chart with fixed data is
an SVG. A chart the reader can filter is an island. That last rung is
...
That's the entire authoring surface for interactivity: import a component that lives next to
the post, add a directive. The component is ordinary React built from the same design tokens as
the site chrome (an island can't invent its own look, so demos age with the theme instead of
...
- It was easier to write in React. (It usually isn't, once you count the hydration.)
