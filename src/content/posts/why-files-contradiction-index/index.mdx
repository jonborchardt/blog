...
- Every **person, organization, place, facility, event, date, role, technology, book/movie, law, ideology** the show has talked about, with per-entity rollups across every episode.
- Every relationship between those entities. Twenty-nine typed predicates: located_in, founded, member_of, authored, debunks, and more — with a transcript span pointer on every single edge so you can jump straight to the moment in the video where the claim was made.
- **Thesis-level claims per episode** — 8–15 per video, each one with a truth score, confidence score, the host's stance toward it (`asserts` / `denies` / `entertains`), expandable evidence quotes, and dependency links to other claims (`supports` / `contradicts` / `presupposes` / `elaborates`).
...
- An **entity graph** with a "color by truth" overlay — nodes and edges shaded red→green based on how the corpus actually treats those claims.
...
```mermaid
flowchart TB
  accTitle: The Why Files Database pipeline
  accDescr: Transcripts are fetched once and cached, then run through GLiNER and GLiREL sidecars for entities and relations, canonicalised with an aliases file, read by an AI session that writes claims, processed by a pure-code reasoning layer into derived truth and contradictions, gated by a 49-signal metrics check, and finally emitted as static JSON for a React site on GitHub Pages.
  A[1. Ingest<br/>transcripts on disk, never refetched] --> B[2. Neural extraction<br/>GLiNER entities + GLiREL relations]
  B --> C[3. Canonicalization<br/>aliases.json + delete-always.ts]
  C --> D[4. AI claim extraction<br/>8-15 claims per video]
  D --> E[5. Reasoning layer<br/>derived truth, contradictions, counterfactuals]
  E --> F[6. Metrics gate<br/>49 signals vs committed baseline]
  F --> G[7. Public site<br/>static JSON + React on GitHub Pages]
  A -.transcript spans on every edge and quote.-> G
```
...
- **GLiNER** for entity extraction. Fourteen typed labels — `person`, `organization`, `location`, `facility`, `event`, `date_time`, `role`, `technology`, `work_of_media`, `law_or_policy`, `ideology`, `quantity`, `creature`, `group_or_movement`. Zero-shot means the labels live in a config file — adding a new one is a one-line change, not a retraining run.
- **GLiREL** for relation extraction. Twenty-nine typed predicates, each with its own confidence threshold tuned against the corpus. Every edge carries the exact sentence span it was extracted from.
...
The fix is a sectioned aliases file (`data/aliases.json`) with seven kinds of overrides: corpus merges, deletions, display-name overrides, per-video merges, per-edge deletions, claim-level truth and field overrides, and operator-authored contradictions and dismissals. An admin UI at `/admin/aliases` lets you curate by hand; a heuristic script proposes the obvious cases automatically; an AI audit pass handles the long tail; and a committed `delete-always.ts` file holds code-level decisions (PR-reviewed, auto-applied on every rebuild) for things that should never be in the graph on this corpus — role-noun persons like "the host," tautologies, transcript artifacts, famous-name short forms like `person:tesla` → `person:nikola tesla`.

### 4. Per-video AI claim extraction

Entities and relations are low-level facts. They tell you "Tesla `worked_at` Westinghouse" but not "the host believes Tesla's wireless power demonstrations were suppressed by financial interests." That second thing is a **claim** — a thesis-level proposition, the kind of proposition that could anchor a Wikipedia section, debatable, with stance and evidence.

For each video, an AI session reads the transcript plus the extracted entities and relations and writes 8–15 claims to `data/claims/<id>.json`. Each claim has:
...
- **Host stance** — `asserts` / `denies` / `entertains`. The `denies` case is important: when the host presents a claim _in order to reject it_ ("some people say X, but…"), the X-claim is extracted with `hostStance: "denies"`. Without this, the cross-video contradiction detector can't tell agreement from rebuttal.
...
- **Broken-presupposition contradictions** — claim B presupposes something claim A denies. Requires truth-asymmetry (A high, B low) so we don't surface chains between two equally-fringe claims.
...
```mermaid
flowchart TB
  accTitle: Cross-video candidate pairs and where each verdict goes
  accDescr: Sentence-transformer embeddings with a Jaccard fallback propose candidate claim pairs. An AI verification pass assigns one of seven verdicts. Logical contradiction, debunks, undercuts and alternative surface on the public contradictions page; same-claim goes to the cross-video agreements page; complementary and irrelevant are dropped; unverified candidates stay admin-only.
  E[Embedding similarity<br/>Jaccard fallback] --> C[Candidate pairs]
  C --> V{AI verification}
  V -->|LOGICAL-CONTRADICTION<br/>DEBUNKS<br/>UNDERCUTS<br/>ALTERNATIVE| P[Public contradictions page]
  V -->|SAME-CLAIM| A[Cross-video agreements page]
  V -->|COMPLEMENTARY<br/>IRRELEVANT| X[Dropped]
  C -.unverified.-> N["Admin only (verified: null)"]
```
...
**Cross-video contradictions need a verification pass.** Embedding-cosine similarity is _good_ at finding "two claims that talk about Tesla" and _bad_ at finding "two claims that disagree about Tesla." The first version of the contradictions page was mostly noise. Adding an AI verification step that verdicts every candidate pair as one of seven kinds before anything surfaces took the false-positive rate from "embarrassing" to "low single digits."

**Operator corrections are high-value feedback.** Every time a human (me, mostly) hits "dismiss this contradiction" or "override this claim's truth" or "merge these two entities," that's signal. The aliases file stores it all, the metrics module counts it all, and the calibration bundle script feeds the most-confirmed examples back as few-shot context for future AI sessions. There's no training step involved, just example injection. But over a few hundred corrections it noticeably improves new extractions.

**Local-first was the right call.** Transcripts and the derived index live on disk under `data/`. The repo doesn't carry the corpus content — only the code, the configs, and a small gold-sample slice. Builds are fast, extraction is offline, and the whole thing works without an internet connection once the transcripts are fetched.

## What's still rough

- **Contradictions you can't trace.** A handful of `manual` contradictions (operator-authored) don't have evidence pointers because I added them before the evidence-mandatory invariant landed.
- **Skeptic scoring is still early.** It works (per-speaker credibility tracks how their claims have held up), but the calibration is rough.
...
- **The "novel links" page** (connections that emerge from combining multiple sources) is gated behind admin while I figure out how to surface them without false positives.

## What you can do with it

Start with an episode you remember well. Open the [home page](https://jonborchardt.github.io/WFD/), pick an episode you remember well, and see if the claim list matches your memory. Open the [contradictions page](https://jonborchardt.github.io/WFD/contradictions) and see if any of them surprise you. Try the [argument map](https://jonborchardt.github.io/WFD/argument-map) seeded by Bigfoot or Roswell or Phantom Time and walk the dependency chains.
