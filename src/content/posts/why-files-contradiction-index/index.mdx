...
hero:
  src: ./hero.png
  alt: "Why Files Database share card: a UFO beams down over dark mountains beside the site title and moon"
draft: false
---
...
![A video page in the database with a story-arc chart plotting each claim's truth over the runtime](./video-page-story-arc.png)
...
![The entities panel: a word cloud of 246 entities coloured by type, with collapsible rows per label](./entities-panel-wordcloud.png)

- **Every entity, with rollups.** Every person, organization, place, facility, event, date, technology, book/movie and ideology the show has talked about, with per-entity rollups across every episode.
- **Every typed relationship.** Twenty-nine predicates (located_in, founded, member_of, authored, denies, funded_by, and more), each edge carrying a transcript span pointer so you can jump straight to the moment in the video where the claim was made.
- **Thesis-level claims per episode.** 8–15 per video as the target, each with a truth score, confidence score, the host's stance toward it (`asserts` / `denies` / `uncertain` / `steelman`), expandable evidence quotes, and dependency links to other claims (`supports` / `contradicts` / `presupposes` / `elaborates`).
- **A contradiction browser.** Pair, broken-presupposition, cross-video and manual kinds, surfacing every place the show contradicts itself and every place one episode's evidence depends on something another episode rejected.
- **A cross-video agreements page.** The flip side, where two different episodes assert the same thesis (positive corroboration).
- **An entity graph coloured by truth.** Edges shade red→green by how the corpus actually treats those claims, while nodes keep their entity-type colours.
...
- **A suggest-an-edit flow.** On every entity, relationship, claim and contradiction, it opens a prefilled GitHub issue with a one-click admin apply link.

<Figure caption="Entity rollup for NASA: 334 mentions across 61 videos, each with the timestamps where it comes up.">
  <Image src={entityRollupNasa} alt="The NASA entity page listing episodes with thumbnails, titles and transcript timestamps" />
</Figure>
...
<Figure caption="The relationship graph around one entity. Every edge is a typed predicate with a span pointer back to the transcript.">
  <Image src={relationshipGraphCarlAllen} alt="A node-link graph centred on Carl Allen, its edges typed member_of, works_for, authored and denies" />
</Figure>
...
<Figure caption="The claims list for an episode: truth and confidence bars, host stance, evidence quotes with timestamps, dependencies and a why? rationale.">
  <Image src={claimsListKersey} alt="Three claim cards from the Kersey time-slip episode, the first expanded to show its full detail" />
</Figure>
...
<Figure caption="The contradictions browser: paired claim cards with stance banners, truth bars, source episodes and shared-entity chips; filters by kind, similarity and publish date on the left.">
  <Image src={contradictionsBrowser} alt="The contradictions page showing 35 corpus contradictions as red DENIES and green ASSERTS card pairs" />
</Figure>

<Figure caption="The argument map seeded from a single claim; dotted edges are shared evidence, coloured nodes are truth.">
  <Image src={argumentMapCia} alt="An argument map: a highlighted central claim about CIA mind control amid coloured claim nodes" />
</Figure>

<Figure caption="Counterfactual mode on a claim: pin its truth to false and watch which dependent claims move.">
  <Image src={counterfactualSlider} alt="A claim card with its slider pinned to false; a related claim drops from 0.90 to 0.54" />
</Figure>

Everything above obeys one rule: **no claim without evidence**. Every relationship, truth judgment, and contradiction carries a transcript id and character span.

![The about page walking from story-arc chart to claim to transcript evidence to contradiction card](./about-page-claim-to-tape.png)
...
A polite YouTube transcript fetcher with rate limiting and a never-refetch rule: once a transcript is on disk it is never re-fetched. The catalog tracks what we have, what's missing, and what stages are stale per video.
...
- **GLiNER** for entity extraction. Thirteen typed labels: `person`, `organization`, `group_or_movement`, `location`, `facility`, `event`, `date_time`, `role`, `technology`, `work_of_media`, `law_or_policy`, `ideology`, `nationality_or_ethnicity`. Zero-shot means the labels live in a config file; adding a new one is a one-line change, not a retraining run. (A few labels, `role` and `law_or_policy`, are extracted and then wholesale dropped at curation; they never reach the public index.)
...
The fix is a sectioned aliases file, `data/aliases.json`. It holds:

- corpus merges and deletions;
- display-name overrides;
- per-video merges and per-edge deletions;
- claim-level truth and field overrides;
- operator-authored contradictions and dismissals;
- curation memory (not-the-same rulings, dismissed clusters).

It is curated three ways: an admin UI at `/admin/aliases` for hand edits, a heuristic script that proposes the obvious cases, and an AI audit pass for the long tail. Alongside it, a committed `delete-always.ts` holds PR-reviewed code-level decisions that auto-apply on every rebuild: things that should never be in the graph (role-noun persons like "the host," tautologies, transcript artifacts), whole labels to drop, and promotions that merge famous-name short forms like `person:tesla` → `person:nikola tesla`.
...
**Cross-video contradictions need a verification pass.** Embedding-cosine similarity is _good_ at finding "two claims that talk about Tesla" and _bad_ at finding "two claims that disagree about Tesla." The first version of the contradictions page was mostly noise. Adding an AI verification step that assigns every candidate pair a verdict of one of seven kinds before anything surfaces took the false-positive rate from "embarrassing" to "low single digits." The verdicts themselves make the point: of 141 candidate pairs the embeddings proposed, the single biggest bucket was two episodes _agreeing_.
...
![An argument map seeded by Bigfoot: a red contradicts edge joins a green claim and a skeptic claim](./argument-map-bigfoot.png)
