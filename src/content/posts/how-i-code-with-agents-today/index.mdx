...
None of this is revolutionary. You are likely already doing this, but I think it's important that we all share what works and what does not work.

For this repo:

- I use LLMs to generate prompts to generate plans.
  - Using LLMs to figure out what assumptions will be made if I don't specify them.
  - Using LLMs to make plans that have validation baked in at every step.
  - Using LLMs to have validation failures explain the why, not just the what.
- I use superpowers for plan shape.
- I use ponytail for simplicity.
- I build scaffolding before any product code.
- I adapt my plans based on what building scaffolding uncovered.
- I use fresh context or other LLMs to gut check that plans are complete.
- When I hit an issue, I bake in validation to prevent any issue I hit.
- I make every stage end with running validation.
- I have every plan a single shippable chunk of work.
...
The rest of this post walks that process end to end on one real case study: this blog, the site you are reading. The process has three beats: decide before you prompt, scaffold and then plan against it, and treat validation as the real output. The result is a static site where, on every push, CI proves that:
...
Whatever the project is; the real product is a repo that agents can maintain without me babysitting. Everything that follows is moving ambiguity out of my head into something checkable.
...
One problem with AI-assisted coding is invisible decisions; they get baked into the first code pass before you notice. Do we want 
- MDX or Markdown
- SPA or static site
- etc...
...
Before asking Claude to code, I used ChatGPT to interrogate the idea and expose hidden architectural decisions. The danger with AI-assisted coding is invisible decisions: they get baked into the first code pass before you notice them. The reusable pattern is simple: do not ask for the implementation prompt until the model has asked you what decisions are missing. My starting point was intentionally messy, a bag of requirements like TypeScript, React, GitHub Pages, a landing page, search, series navigation, a local admin, light/dark mode, accessibility, etc.

I did **not** ask for the final Claude prompt immediately. Instead I asked ChatGPT, before writing the actual startup prompt, to ask me the questions that would be useful for making the architecture explicit. That changed the task from "write a prompt" to "surface the decisions this prompt needs to encode." ChatGPT came back with 45 questions, and about ten of them were decisions I did not know I needed to make.
...
By the end of the conversation, the fuzzy idea was a decision list, and a decision list is something I can build prompts from. It became two: one prompt to build the scaffold, and one to build the implementation plan for the product.
...
The prompt bakes in acceptance criteria that are all checkable ("MDX renders, an island hydrates, URLs work under the base path"), so the agent can verify itself instead of guessing. 

First checks are locked in before any product exists.
...
I started a brand new session to audit the scaffold, because the session that built it would defend its own assumptions. Fresh eyes found defects: 
- typography plugin never registered
- dates off by one
- etc...
...
Now I run the plan building prompt to generate eleven ordered plans, each independently shippable. Every plan carries its own acceptance criteria and validation section, so "done" stays checkable at every stage, not just at the end.

←

</SpeakerNote>

The second prompt turned the real repo into an ordered sequence of eleven bounded implementation plans. It told Claude to inspect the repo, find the gaps between scaffold and product, and turn them into implementation plans that kept the repo shippable at every stage. Every plan had to carry the same sections:
...
I lean heavily on superpowers for plan shape and ponytail for simplicity.
...
I ask the LLM: "can each plan execute in a brand new context with zero memory of this chat?" 

If it cannot, I fix it by having the LLM embed the relevant parts of the spec as a quote not a summarization. Summarization is where your invariants quietly die.
...
Then I run the plans. Eighty minutes, eleven commits, shippable after every one. Shippable for real: each plan ran the validation and build before its commit. 

The test suite grows while the product does.

Execution was boring because the stages before it were not. If coding is the exciting part of your agent story, the prep was too thin.

←

</SpeakerNote>

With the plan reviewed, I opened a fresh Claude session and pointed it at the repo. Then it ran, largely unattended, for about 80 minutes. It read the plan, inspected the repo, and worked through the tasks: the foundation fixes, the design system, the article page, the MDX primitives, build-time diagrams, OG cards, validation checks, search, the dev-only admin, etc. Each stage ran the validation and build before moving on, so the repo stayed shippable the whole way through. The git log has the receipts; [An Agent Built This Blog](/an-agent-built-this-blog/) walks through them commit by commit. The execution itself was boring, and that is the point: if coding is the exciting part of your agent story, the preparation was too thin.
...
The result was a high quality system. One run delivered most of the product: 
- design system
- article pages
- MDX primitives
- OG cards
...
Checks were never a phase: the scaffold prompt baked in the testing stack and every plan carried its own validation section. 

For example 
- tags compile to TypeScript unions, so tag drift is a type error, not next quarter's cleanup
- a validation error becomes an instruction for the fixing agent, not just a complaint.
...
By this point, enough checks have accumulated that quality is a floor, not a target: a green build MEANS 
- Lighthouse 100s
- axe-clean 
- no broken links
- etc... 

A red check blocks deploy. 

And the repo ended with operating procedures, not just code: 
- CLAUDE.md says what must stay true
- skills say how recurring work is done
- validation says what cannot be wrong
