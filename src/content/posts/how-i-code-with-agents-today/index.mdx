...
I want to share with you a project I did a few weekends ago. I used a pattern that turned out really well.

~ Interrogate the AI until a fuzzy idea becomes decisions.

~ Build scaffolding before product.

~ Let the coded repo dictate plan changes.

~ Hand the plan to the agent; review reports and reasons.

~ Design checks directly into the build.

~ Make almost free extensions to finish up.

Every stage ends with something the repo can enforce that it could not before.
...
The rest of this post walks that process end to end on one real case study: this blog, the site you are reading. The process has three beats: decide before you prompt, let reality replace the prompt, and treat validation as the real output. The result is a static site where, on every push, CI proves that:
...
So we start with a fuzzy idea. This is the point of maximum ambiguity. Everything about this project lives in my head and nowhere else.

The blog is just a convenient product; the real product is a repo that agents can maintain without me babysitting. Everything that follows is moving ambiguity out of my head into something checkable.
...
I start with an interrogation. The trick is to not ask for a prompt, ask what questions need answering first. In this case 45 came back; about ten were decisions I did not know I needed to make.

One problem with AI-assisted coding is invisible decisions; they get baked into the first code pass before you notice. In this example I worked through MDX vs Markdown, SPA vs static, etc.
...
I decide invariants, the agent decides implementation mechanics. "/admin/ must not exist in production" is mine; how Astro guarantees it is not. 

By the end of this conversation the fuzzy idea is a decision list. And that is something I can build prompts from.

~ One prompt for building the scaffolding.

~ One prompt for building the plan for the product.
...
I build scaffolding before, and separately from, the product. The first prompt explicitly refuses to build the blog, skeleton only, and has written non-goals, because agents drift toward finishing whatever looks unfinished.

I bake in acceptance criteria that are all checkable ("MDX renders, an island hydrates, URLs work under the base path"), so the agent can verify itself instead of guessing. First checks are locked in before any product exists.
...
I run the scaffold prompt and get back a report based on reality. The report is not a status update, it is input to the next reasoning step, so what I want is deviations with reasons.

"skipped it" is useless; "skipped because it does not support ESLint 10, and axe covers it" tells the next agent when to revisit.

I paste the report back to ChatGPT or a fresh Claude context to see if the plan building prompt needs updating based on the actual repo choices.
...
Up to here, authority lived in my head and the prompt. From here on, the repo is the source of truth and nothing trusts memory again.

I started a brand new session to audit the scaffold, because the session that built it would defend its own assumptions. Fresh eyes found defects: typography plugin never registered, dates off by one, etc.
...
Now I run the plan building prompt to generate eleven ordered plans, each independently shippable. The checks thread continues here: every plan carries its own acceptance criteria and validation section, so "done" stays checkable at every stage, not just at the end.

The test I always run: "can each plan execute in a brand new context with zero memory of this chat?" If it cannot, fix it by embedding the spec and a quote-don't-summarize rule. Summarization is where invariants quietly die.

Here I lean on superpowers for plan shape and ponytail for simplicity.
...
Once again I used a second model to review the plans. It tightens: no opportunistic refactors, no replanning. Reviewers tighten; they do not redesign.

This is checks and balances on the process itself, not just the code. Every layer so far has had its own reviewer.
...
Then I run the plans. Eighty minutes, eleven commits, shippable after every one. Shippable is not a vibe: each plan ran the validation and build before its commit, and one entire plan added nothing but new build-output checks. The check suite was growing while the product was.

Execution was boring because the stages before it were not. If the demo magic is the exciting part of your agent story, the prep was too thin.
...
I feed the final report from Claude back to the model that did the first interrogation, and it judges the outcome against the original intent. The builder reports with reasons, the reviewer checks them against the plan.
...
The result was a high quality system. One run delivered most of the product: design system, article pages, MDX primitives, OG cards, search, the dev-only admin, and the skills. Walk the table; every row was one plan, one commit.
...
Checks were never a phase: the scaffold prompt baked in the testing stack, every plan carried its own validation section, and one plan was nothing but build-output checks. For example tags compile to TypeScript unions, so tag drift is a type error, not next quarter's cleanup; and a validation error becomes an instruction for the fixing agent, not just a complaint.
...
The layered model is that `CLAUDE.md` invariants say what must stay true, skills say how recurring work is done, and validation says what cannot be wrong. This is the end state I want in agent-maintained repos; [Repos as Experts](/repos-as-experts/) explores it further.

## Quality became a floor, not a target

<SpeakerNote>

By this point the checks have compounded into a floor: a green build MEANS Lighthouse 100s, axe-clean, zero JS on prose, no broken links. A red check blocks deploy. And the repo ended with operating procedures, not just code: CLAUDE.md says what must stay true, skills say how recurring work is done, validation says what cannot be wrong.
...
My review attention moved up: did the architecture survive, did validation get stronger, did JS stay justified, did scope stay bounded.
...
At this point extensions are now almost free. TOC, lightbox, print styles, even the speaker notes. Each was a one-or-two-sentence prompt against a repo that knows its own rules. 

They can be short because every check from the earlier stages applies to the change the same way, whether it lands in plan one or session forty. The validation investment pays dividends forever.

When an agent hits a class of mistake, turn it into a test, a check, a rule, or a skill, so that every session makes the next session's mistakes harder to commit.
...
Extension was cheap because validation was the real output; validation could accumulate because the repo replaced the prompt; the repo could replace the prompt because the deciding happened before any prompting. 

The win was never a better giant prompt; it was knowledge leaving the chat and becoming code, schemas, tests, and skills.
