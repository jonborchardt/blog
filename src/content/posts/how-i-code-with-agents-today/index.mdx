...
The main shift is not "Claude writes more code"; it is that I am designing a system where future agent sessions are less likely to do the wrong thing.

The process has warts — at one point Claude overran scope — and I show those too.

---

## The case study

The concrete example was a new static technical blog called **Always Shippable** — an intentionally modest app; the workflow is the interesting part.
...
Not a huge application, but complex enough — routing, static generation, MDX, React islands, client-side search, SEO, accessibility, dev-only admin tooling, GitHub Pages deployment constraints — to exercise the process.

---

## The important part was not the blog

The blog was just a convenient product; the real object being built was an agent-maintainable codebase — agentic coding increases the value of executable constraints.

### Agent-maintainable means
...
Before asking Claude to code, I used ChatGPT to interrogate the idea and expose hidden architectural decisions. The reusable pattern: do not ask for the implementation prompt until the model has asked what decisions are missing.

### The initial input was intentionally messy
...
If I had skipped this step, Claude would have silently made those choices — some fine, some wrong. The problem is not just wrong decisions; it is invisible ones.
...
I made product and architecture calls, but avoided over-prescribing implementation details: the human decides invariants; the agent decides implementation mechanics while executing.

### Examples of decisions I locked
...
### Examples of things I did not need to dictate
...
The stack became **Astro + React + MDX + TypeScript + Tailwind + shadcn/ui + Radix + npm** — a crisp architecture boundary future agents can preserve.
...
The `/blog/` deployment path was a major constraint, and we handled it as a deployment base, not a source-folder structure; it later became a validation target.
...
### Why this was worth settling early
...
Posts are self-contained MDX directories with colocated assets and optional post-specific components — allow local weirdness, keep shared abstractions clean.
...
### Why this matters for agent-authored content
...
The first Claude prompt explicitly said the goal was **not** to build the finished blog — the most important prompt-design decision in the whole process.

### The key opening constraint
...
Checkable acceptance criteria cause the agent to verify; vague quality goals cause it to guess — a takeaway that applies to any prompt that hands work to an agent.
...
Claude first inspected the environment, versions, package compatibility, and git state — a good agent workflow forces inspection of the live environment.
...
None of these were good candidates for pre-planning — which is why we scaffold before writing the full implementation plan.

---

## Claude tried to break its own scaffold

Verification went beyond running the obvious tests: if a rule matters, create a deliberate violation and make sure the system rejects it.

### Examples of adversarial checks
...
The quality of the final report mattered because it became input to the next reasoning step — the next model needs reasons, not just a status report.

### The three useful report categories
...
"Skipped jsx-a11y" is ambiguous. "Skipped jsx-a11y because it does not support ESLint 10 yet, while axe e2e covers rendered accessibility" tells the next agent when that decision should be revisited.
...
After the scaffold existed, I stopped treating the prompt as authoritative and made the repository authoritative: prompt authority decays, repo authority increases.
...
### The key instruction in Prompt 2
...
### Four defects the planning session found
...
The scaffold session had shipped all four defects. Not because it was careless, but because each one was a silent gap: everything green, output subtly wrong.
...
Claude did not invent the planning methodology from scratch; it used the existing process skills I had installed. Reusable process assets compound; skills are process memory.

### What the planning skill contributed
...
Before running the plan, I brought it back to ChatGPT for an independent review — as a reviewer, not an infinite redesign loop.

The review kept the plan intact and added execution guardrails: one plan at a time, each independently shippable, no opportunistic refactors, validation against a known production preview, one commit per plan. The full note is in the transcript below.
...
Each plan should leave the repo shippable and create a reviewable checkpoint — the same reason we prefer small PRs; agents make this more important, not less.
...
### Why this is still a failure mode
...
Despite the scope overrun, the resulting repo was close to complete and had a strong validation posture — a strong outcome that should not erase the scope-control lesson.
...
The site avoided turning into a React app with static-looking pages. Checking that meant reviewing system properties, not just features.

### Signals that static-first held
...
Validation turns architectural and content rules into build failures future agents cannot ignore — probably the most reusable lesson here for a team.
...
A validation error should be an instruction, not just a complaint: it should tell the next agent exactly how to fix the problem.

As agent autonomy increases, executable constraints become more valuable than review comments — review catches bad states after they exist; validation prevents them from shipping.

---

## The repo ended with its own skills

The process skills used to build the repo were transformed into repo-specific content skills: the repo now has operating procedures, not just code.
...
A green build now implies accessibility, SEO, links, metadata, base paths, and production-safety checks passed. This is where the process compounds.
...
### What this means for team practice
