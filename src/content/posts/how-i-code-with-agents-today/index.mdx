...
Now that we had some idea of the architecture, it was time to start building, but not the features. I always split project setup from implementation. The first prompt on any project is about the skeleton only: architecture, dependencies, file structure, contracts, tooling, and validation, with no features. Features come later, in separate prompts, once the foundation is in place and the agent has something solid to build on. So the first Claude prompt explicitly said the goal was **not** to build the finished blog. The opening constraint read:
...
## Claude builds, reports, and ChatGPT reviews the report

With the plan reviewed, I opened a fresh Claude session and pointed it at the repo. Then it ran, largely unattended, for about 70 minutes. It read the plan, inspected the repo, and worked through the tasks: the foundation fixes, the design system, the article page, the MDX primitives, build-time diagrams, OG cards, validation checks, search, the dev-only admin, etc. Each stage ran the validation and build before moving on, so the repo stayed shippable the whole way through.

When it stopped, it produced the same kind of report as the scaffold session: what it verified, where it deviated from the plan and why, and what it marked as a deliberate shortcut. I did not read that report alone. I pasted it back into the ChatGPT thread that had reviewed the plans, so the model that knew the product intent could judge the outcome against it. That is the same loop as before, just one turn later: Claude does the work, the report carries the reasons, and a second model with product context checks whether the work still matches the plan.
