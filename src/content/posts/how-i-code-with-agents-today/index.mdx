...
The rest of this post walks that process end to end on one real case study: the blog you are reading. It starts from a rough idea and a conversation, goes through a bootstrap prompt, a scaffold, a repository-aware plan, and an agent run, and ends with a static site that scores Lighthouse 100/100/100/100 on representative pages, passes an axe WCAG 2.2 AA audit and the W3C Nu markup validator on every page, ships zero JavaScript on prose pages, and fails its own build on a broken link, a missing alt, an unknown tag, or any admin output in `dist/`. Every one of those checks runs in CI on every push, so the quality described at the end is a floor the repo enforces, not a target I remember to aim for.
