...
hero:
  src: ./hero.png
  alt: "A tangled idea unwinds through a two-model conversation and a written plan into a repository of ordered blocks behind a validation check"
draft: true
---
...
Agentic coding isn't "write a giant prompt and hope." It's an engineering process for moving ambiguity out of my head and into code, validation, plans, and repository rules, in roughly that order:

<ProcessMarker step={1} />

The main shift isn't that Claude writes more code. It's that I'm designing a system in which future agent sessions are less likely to do the wrong thing, because the repo itself pushes back. The process has warts: at one point Claude ignored a stop instruction and implemented all eleven plans in one go. That's in here too.

The rest of this post walks that process end to end on one real case study: the blog you are reading. It starts from a rough idea and a conversation, goes through a bootstrap prompt, a scaffold, a repository-aware plan, an agent run, and the short sessions that extended it afterwards, and ends with a static site that scores Lighthouse 100/100/100/100 on representative pages, passes an axe WCAG 2.2 AA audit and the W3C Nu markup validator on every page, ships zero JavaScript on prose pages, and fails its own build on a broken link, a missing alt, an unknown tag, or any admin output in `dist/`. Every one of those checks runs in CI on every push, so the quality described at the end is a floor the repo enforces, not a target I remember to aim for.

## The case study

The concrete example is a new static technical blog called **Always Shippable**, the one you are reading. The app is intentionally modest; the workflow is the interesting part. The core constraints were a GitHub Pages project site served under `/blog/`, static pages, MDX posts, React only for interactive islands, search with no external service, a local-only admin, and no analytics, comments, CMS, or backend, all while meeting accessibility, SEO, performance, and responsive-behavior targets. That is not a huge application, but routing, static generation, MDX, React islands, client-side search, SEO, accessibility, dev-only admin tooling, and GitHub Pages deployment constraints are enough moving parts to exercise the process.

The blog was just a convenient product. The real object being built was an agent-maintainable codebase, and agentic coding raises the value of executable constraints. "Agent-maintainable" meant explicit contracts, typed schemas, deterministic file organization, validation, reusable primitives, build-time failures for invalid states, straightforward conventions, low ambiguity, and clear instructions for future agents. Instead of relying on future agents to remember prose instructions, I want the repo itself to reject bad states. Each human convention should become a repository contract, and each contract should become a machine-checkable invariant. In practice that means an unknown tag is a build failure, a broken internal link is a validation failure, and admin content in `dist/` is a build failure.

<ProcessMarker step={2} />

## Step 1: I talked to ChatGPT before Claude

Before asking Claude to code, I used ChatGPT to interrogate the idea and expose hidden architectural decisions. The reusable pattern is simple: do not ask for the implementation prompt until the model has asked you what decisions are missing. My starting point was intentionally messy, a bag of requirements like TypeScript, React, Vite or equivalent, ESLint, GitHub Pages, a landing page with featured content, search, individual posts, series with next/previous navigation, a local admin, logo and favicon, accessibility and contrast, light/dark mode, static content, and React/SVG/images inside posts.
...
I did not need to dictate the exact Node version, the exact Astro APIs, the exact component implementation, how the local admin writes files, the shape of test files, every CSS token, or every reusable primitive. The invariant mattered more than the implementation. "`/admin/` must not exist in production output" is an invariant; the precise Astro/Vite mechanism that guarantees it is an implementation detail, and the agent is better placed than I am to pick it once it can see the real toolchain.
...
The stack became **Astro + React + MDX + TypeScript + Tailwind + shadcn/ui + Radix + npm**, which gives future agents a crisp architecture boundary to preserve. Astro owns pages, routing, layouts, content loading, static rendering, metadata, and build-time behavior. React owns interactive islands, interactive visualizations, interactive demos, selected client-side tools, and bespoke interactive MDX components. The non-goals were just as important and were written down explicitly: do not turn the site into a React SPA, do not introduce React Router, do not hydrate the entire page tree, and keep static content as static HTML wherever possible.
...
(`/archive/` later became `/all-posts/`.)
...
The first Claude prompt explicitly said the goal was **not** to build the finished blog. That was the most important prompt-design decision in the whole process. The opening constraint read:
...
What had to be true at the end was concrete:
...
- the repo structure is ready for future Claude skills and agent-authored posts
...
Checkable acceptance criteria cause the agent to verify; vague quality goals cause it to guess. This applies to any prompt that hands work to an agent. From the agent's side, "build me a great blog" forces it to silently make dozens of decisions and hope, whereas "MDX must statically render, a React island must hydrate, URLs must work under `/blog/`, the build must pass" gives it checkable truths. It can run commands and know whether it is done. The phrasing I keep coming back to is that "what must be true" is checkable and "build something great" is not.

<ProcessMarker step={4} />

## Claude's first action was not coding

Claude first inspected the environment, versions, package compatibility, and git state. A good agent workflow forces inspection of the live environment before anything else. It started with the boring commands:
...
That turned up a real problem: the installed Node was 20.19, and the latest Astro was 7.x, which requires Node 22.12 or newer. Claude also recognized that Astro 7 was newer than its training data. So it queried npm for current versions and peer dependencies, checked the Astro upgrade notes instead of trusting memory, found nvm-windows, installed Node 24 LTS, and pinned Node in `.nvmrc`.

A model's built-in knowledge of a fast-moving ecosystem is always stale, and the process compensates by making checking cheaper than guessing. The prompt did not say which Node version to use. It said to use current mutually compatible stable versions, run the relevant commands yourself, fix errors you encounter, and do not stop after generating files. That wording selects for verification instead of plausible generation.

## The environment fought back

Most scaffold problems were not product problems; they were real toolchain collisions. `eslint-plugin-jsx-a11y` did not support ESLint 10 yet, and Claude skipped it rather than downgrading ESLint, because axe-based e2e already covered rendered accessibility. `shadcn init` failed because the minimal Astro template lacked the path alias shadcn expected, so Claude added the alias first and then shadcn worked. Shell heredoc writes were mangled because backticks and `${...}` were expanded by the shell wrapper before reaching files, and Claude switched to structured file writes. Astro detected an agent environment and auto-backgrounded `dev` and `preview`, which Claude figured out by reading `node_modules`. And an `nvm use` call hit a hung elevation prompt that disrupted the local Node junction, which later affected my own terminal; the transcript made that diagnosable.

None of these were good candidates for pre-planning, which is exactly why we scaffold before writing the full implementation plan. You cannot plan around problems you have not met yet.

## Claude tried to break its own scaffold

Verification went beyond running the obvious tests. The principle is that if a rule matters, you create a deliberate violation and make sure the system rejects it. Claude grepped the built `dist/` HTML for base-path correctness, checked canonical URLs and JSON-LD, and checked what scripts static pages actually shipped. It found an RSS channel link missing the `/blog/` base. It created a temporary draft post and verified it was absent from production pages, RSS, and the sitemap, created a deliberately invalid post with an unknown tag and series and verified the build failed, and confirmed `admin/` was absent from `dist/`.

For agent-authored repos, validation exists to stop future agents, so the agent should test whether validation stops **itself**. That is a stronger check than proving the happy path works.

## The scaffold report separated facts, deviations, and deferred work

The quality of the final report mattered because it became input to the next reasoning step, and the next model needs reasons, not just a status. The report had three useful categories. Verified facts: which commands ran, what passed, what was generated. Deviations with reasons: Node 24 because Astro 7 required newer Node, no jsx-a11y because of the ESLint 10 incompatibility, a system font stack instead of Geist, agent-backgrounded preview behavior, and a repo that was initialized but uncommitted. Explicitly deferred work: search, the full admin, OG images, rich primitives, and polished design.
...
A clean inspection pass found silent defects that green tests did not catch. `@tailwindcss/typography` was installed but not registered, so no `.prose` CSS existed. Shiki dual-theme was configured, but the dark-mode CSS hook did not exist. UTC frontmatter dates rendered one day early in local time. And Playwright was testing the wrong server because a stale dev server had reused the port.

This worked because the fresh session had no memory of building the scaffold, an explicit instruction to distrust it, permission to inspect the real repo, and a mandate to put foundational fixes early. Those issues became **Plan 1: Foundation fixes** instead of getting buried under feature work. Fresh sessions are not a formality; they are a review mechanism that reduces the chance the agent rationalizes or overlooks its own prior work. The scaffold session had shipped all four defects, not because it was careless but because each one was a silent gap: everything green, output subtly wrong.

<ProcessMarker step={6} />

## Prompt 2 produced plans, not code

The second prompt turned the real repo into an ordered sequence of bounded implementation plans. It told Claude to deeply inspect the existing repository, understand what is actually implemented, identify gaps between scaffold and intended product, determine dependencies, produce concrete implementation plans in a sensible order, and keep the repo working and shippable after every meaningful stage. Every plan had to carry the same sections:
...
The "explicitly out of scope" section is not decoration. It is a scope-control device for agents, and as you will see shortly, even that was not quite enough.
...
Claude did not invent the planning methodology from scratch; it used the process skills I already had installed. The planning skill prescribed that plans assume the executor has zero context, that file responsibilities are mapped before tasks, that tasks are sized to carry their own test cycle, that plans follow DRY/YAGNI/TDD expectations, that commits are frequent, and that plans are saved in a standard location. Reusable process assets compound. Skills are process memory.

The project goal and the process skill aligned neatly. The product wanted an agent-authored repository with explicit contracts and validation, and the planning skill produced explicit contracts for future agent sessions. It is the same philosophy at two levels: repo contracts help agents maintain code, and plan contracts help agents execute work.

## The plan had to survive without the chat

Anything future agents need has to move out of ephemeral chat context and into durable repository or memory artifacts. Claude found a gap here: the plan referenced the planning prompt itself, which was unsafe because a future session might receive only "Implement Plan 4" and never see the original product prompt. The missing durable content included the product identity, the non-goals, the admin fields, the quality targets, and the agent-authored repository philosophy.

The fix was to add a condensed product spec into the plan document itself, plus a subagent briefing rule that subagent prompts must quote, not summarize, the global constraints, execution notes, product spec, and full plan text. Why quote instead of summarize? Because summarization is where invariants die.

<ProcessMarker step={7} />

## I reviewed the plan with ChatGPT

Before running the plan, I brought it back to ChatGPT for an independent review, as a reviewer and not as an infinite redesign loop. The review kept the plan intact and added execution guardrails: one plan at a time, each independently shippable, no opportunistic refactors, validation against a known production preview, one commit per plan. A second model is useful for catching overengineering, detecting architecture drift, challenging speculative fallbacks, strengthening task boundaries, and checking sequencing. But once the repository-aware plan is coherent, wholesale replanning is usually counterproductive. The full note is in the transcript below.
...
## Execution should have been one plan at a time

Each plan should leave the repo shippable and create a reviewable checkpoint. It is the same reason we prefer small PRs, and agents make it more important, not less. The intended loop was simple: implement Plan 1, validate, commit, stop; then implement Plan 2, validate, commit, stop; and so on through the program, with me reviewing between each step.

## Claude overran the boundary

Claude implemented all 11 plans instead of stopping after Plan 1. Its report read:
...
That is productive. It is also not what was requested. Momentum is the agent failure mode that looks most like success: each next plan was well-specified, the repo was green, and continuing looked locally reasonable at every step. But it removed the human review gate between plans, which was the whole point of splitting the work. Next time the wording will be tighter:
...
<Details summary="Actual conversation: Claude after implementation">
...
## The final implementation was strong

Despite the scope overrun, the resulting repo was close to complete and had a strong validation posture. That is a good outcome, and it should not erase the scope-control lesson. Here is what each plan delivered:
...
The deviations were the interesting part. Prettier ignores `*.mdx` because the MDX formatter mangled JSX-embedded lists and fences. Markdown plugins moved to Astro 7's actual processor mechanism. Admin routing stayed structured to guarantee no production output. Additional support dependencies were added only where required, and some explicitly marked shortcuts remained and were not automatically fixed.

<ProcessMarker step={10} />

## The initial run was not the end

The eleven plans did not produce a finished site, and I did not expect them to. Over the next several sessions I went back in and added what the program had not covered: the logo was redesigned, the skills gained authority and sourcing rules, posts got suggested next reading, a table of contents, and read time, this post got its process rail, the site gained an `llms.txt` index, an external link checker, an image lightbox, git-derived last-updated dates, a print stylesheet, a legal page, and a tag-pill fix for WCAG target size.

None of it was challenging, and none of it caused an issue. That is the point. Each feature was a short session against a repository that already knew how to reject a broken state: the schema caught bad frontmatter, the dist checks caught a missing alt or a link outside the base path, the e2e suite caught a layout regression, and the skills routed each change through the same validate-and-build sequence as the original plans. The cost of the validation work in the first afternoon was paid back on every one of these additions, because I could ask for a feature, read the diff, and trust the green build for everything I did not read.

None of them needed a long prompt either. Each was one or two sentences against a repo that already knew its own rules, and in hindsight any of them could have been appended to Prompt 1 as-is. These are roughly what I typed.
...
## Static-first survived

The site avoided turning into a React app with static-looking pages. Checking that meant reviewing system properties rather than features: build-time Mermaid, static math, a static table of contents, static article primitives and pages, a lazy search index, a single archive/search island, explicit JS budget tests, and no full-site hydration, React Router, or backend. The architecture was never just "make the pages look right." It was a specific assignment of each concern to a rendering mode:

| Concern | Rendering |
|---|---|
| Prose, navigation, article layout | static HTML |
| SVG illustration | static HTML/SVG |
| Interactive chart, interactive demo, search/archive | React island |
| Theme control | tiny client behavior |

That is the kind of distinction agents need to preserve, and the kind that quietly erodes if nobody checks it.

## Validation became the highest-leverage output

Validation turns architectural and content rules into build failures future agents cannot ignore. Of everything here, this is probably the most reusable lesson for a team. Build failures now catch:
...
A green build now implies that accessibility, SEO, links, metadata, base paths, and production-safety checks all passed. This is where the process compounds. Early on, quality depended on the model reading a prose instruction, remembering it, caring about it, and a human noticing if it forgot. By the end, quality depended on the schema rejecting invalid content, validation rejecting broken output, e2e checking rendered behavior, and skills routing work through the right sequence. Claude's own framing of it was:
...
My review target moves up from local code style to architecture, scope, validation, and maintainability. The first question is whether the architecture survived: static content stayed static, React remained narrow, the `/blog/` base path stayed correct, posts remained MDX, post-local components remained possible, no backend crept in, and the local admin stayed out of production. Then, did validation get stronger? If the agent added features but weakened validation, that is a bad trade. Did JavaScript stay justified, with normal articles not turning into interactive applications? Were the deviations reasonable, meaning they preserved invariants while adapting to reality? And did scope stay bounded? Correct code outside the requested boundary is still a process failure.

## Make future mistakes harder to commit

Every agent session should leave behind stronger constraints, not just more code. In this run:

- the shell write issue became a `CLAUDE.md` rule
- the port collision became a validation/preview rule
- base-path issues became dist checks
- draft behavior and tag/series consistency became schema and validation
- admin non-shipping became a production dist check
- the content workflow became skills
