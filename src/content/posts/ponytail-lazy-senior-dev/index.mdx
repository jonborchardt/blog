...
hero:
  src: ./hero.png
  alt: A seven-rung ladder where the top rungs branch to stop markers and only the bottom leads to code
draft: false
---
...
import LadderViz from "./ladder-viz.svg";
import PrReplayViz from "./pr-replay-viz.svg";

[WorldLock](/worldlock-ar-window/) was written with Claude Code running a mode called **<a href="https://github.com/DietrichGebert/ponytail" target="_blank" rel="noopener">ponytail</a>**. The pitch: the AI role-plays a lazy senior developer. Lazy meaning efficient, not careless: the kind of engineer who has been paged at 3am for someone else's clever abstraction and now believes the best code is the code never written.
...
<VizFigure
  name="The ponytail ladder as a chain of early exits"
  summary="Seven questions asked in order before writing code, drawn as a ladder. Each rung that holds exits immediately, skipping the work entirely or reusing what the codebase, standard library, platform, or an installed dependency already provides, down to a one-liner. Only when every rung fails does the model write the minimum code that works, and any deliberate corner it cuts there gets a ponytail comment naming the ceiling."
>
  <LadderViz class="mx-auto h-auto w-full max-w-2xl" aria-hidden="true" />
</VizFigure>
...
- **The debt is labeled.** Search the repo for `ponytail:` and you get the full list: four comments, in-place, greppable. That's the whole backlog of deferred work.
...
Fingerprints on one hobby codebase are suggestive, not proof. So I've also been testing ponytail alongside Superpowers on my own projects, including a retrospective test against a real merged PR. The method: check out the code at the commit immediately before each issue was raised or fixed, run the review lenses cold without describing the expected finding, and compare against what generic AI review and human review actually caught at the time. The same underlying model ran throughout, so the only real variable was the review skill, its instructions, and its context. These were replays of actual review history, not constructed examples.

<VizFigure
  name="How the PR replay was run"
  summary="For each issue in a merged PR's history, the repository is checked out at the commit just before that issue was raised or fixed. Each review lens (ponytail, Superpowers, and a generic AI review) is run cold on that snapshot with the same underlying model and no hint of the expected finding. Its output is then compared with what the generic AI review and the human reviewers actually recorded at the time."
>
  <PrReplayViz class="mx-auto h-auto w-full max-w-2xl" aria-hidden="true" />
</VizFigure>
