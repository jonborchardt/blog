...
hero:
  src: ./hero.png
  alt: A fan of thin wealth lines, one per start year; one sinks to zero, another climbs to $5.8 million
draft: false
---
...
import ReplayFanViz from "./replay-fan-viz.svg";
...
<VizFigure
  name="How the simulator turns one plan into ninety-eight lives"
  summary="One plan is replayed once per historical start year from 1928 to 2025, each run using the actual stock, bond and inflation sequence that followed that year. The result is ninety-eight balance paths, reported together as a fan in today's dollars."
>
  <ReplayFanViz class="mx-auto h-auto w-full max-w-2xl" aria-hidden="true" />
</VizFigure>

![Baseline-plan fan chart, 2026 to 2057: faint per-year lines, a percentile band, a rising median](./fan-chart-baseline.png)

## Four of those ninety-eight lives, all in today's dollars

<Figure caption="The four lives below on one chart: real portfolio balance by year of retirement. Dots are the balances stated in the text; the lines between them are straight, not the actual year-by-year path.">
  <VizFigure
    ...
    data={{
      ...
      rows: [
        ...
        ["15", "n/a", "$319k", "$3,500k", "$740k"],
        ["20", "≈$520k", "$291k", "n/a", "$870k"],
        ["25", "n/a", "$156k", "n/a", "n/a"],
...
Two retirees, sixteen years apart, identical in every decision, one broke and one rich. Following the same good plan, they got those outcomes; each could have done better by deviating, but the right deviations were opposites (the 1966 retiree needed to cut spending early and hard, the 1982 retiree could have spent freely from day one), and nothing in the plan, or in the world of 1966 or 1982, told them which retiree they were. That's the risk. Not "markets go down." _Your retirement date is a lottery ticket and you draw it once._
...
Every experiment I've run on my own plan was, in retrospect, this: find the cohorts that break the plan, and shift the shape of the distribution so those lines stay above zero, even at the cost of the top of the fan. For my plan the failing family was the inflation cohorts, so the fix was an asset that holds real value through a decade of inflation, held only in the window around the retirement date where the sequence matters. It happened to raise the median too, but that's not why it won: it cut off the zero tail. That's the trade you want, even when it isn't free.
...
And the third is time, meaning working longer. Each extra working year adds savings, removes a year of withdrawals, and lifts every line in the fan. But its value has a cliff: while some start years still fail, one more year can erase whole failure cohorts; once every start year survives, another year of work buys no safety at all, just a bigger pile.
