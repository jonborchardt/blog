...
hero:
  src: ./hero.png
  alt: A fan of thin lines spreading out from one starting dollar amount, one line per retirement year; a warm line sinks to zero on the left while a blue line climbs to the top right
draft: false
---
...
Every experiment I've run on my own plan was, in retrospect, this: find the cohorts that break the plan, and shift the shape of the distribution so those lines stay above zero — even at the cost of the top of the fan, which was never spendable anyway. For my plan the failing family was the inflation cohorts, so the fix was an asset that holds real value through a decade of inflation, held only in the window around the retirement date where the sequence matters. It didn't raise the median. It cut off the zero tail. That's the trade you want.
...
_The textbook household above is synthetic — the point is the shape, not the dollars. Runs that outlive the historical dataset continue on the long-run average year, so the most recent cohorts are partly projection; the older ones are entirely history._
