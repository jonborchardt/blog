---
title: "omyac-upscaler: all the way into the anchor graph"
description: Porting agi-up's anchor-graph upscaler to SCI0 PICs — four colour-model fixes, one convention fix, and why snap-blend is leaving the PIC path.
...
hero:
  src: ./hero.png
  alt: Pixel cells with a stair-stepped block line on the left and a smooth anchor-dot stroke on the right
draft: false
---
...
For the past few months, the SCI0 PIC pipeline has mostly been a story of incremental refinement. [`snap-blend`](/sci0-pic-upscaling-native-render-oracle/) improved pixel-space scaling with leak-detection heuristics, dither-aware resolution, and a dual-resolution adaptive guard (the native render supervising the upscaled one, with rollback and fallback). Each iteration fixed a real class of artefacts. Each iteration also hit the same ceiling: pixels alone only carry so much information.
...
This post documents the SCI0 port, the five engine-specific fixes that made it viable, and why the project is moving its PIC visual path away from `snap-blend`. (A [sibling post](/agi-up-as-a-reference-renderer/) covers using `agi-up` as a reference renderer; this one is about making its algorithm native.)

## Why port at all

<Figure caption="snap-blend (left) against omyac-upscaler (right) on the same PIC; the wipe sweeps back and forth so the same region can be read under both." zoom>
  <img
    src={sweepSnap.src}
    width="1920"
    height="1140"
    alt="Wipe between snap-blend and omyac renders of a wrecked spacecraft on grey paving under blue walls"
  />
</Figure>

`snap-v5-blend` already ships and renders. The omyac upscaler was initially registered alongside it as a separate strategy so the pic-dev sweep page could compare them side-by-side on the same PICs, a hedge in case omyac didn't pan out. The hedge wasn't needed. Once the five fixes below were in, omyac was better on every PIC I've put it against. The project is moving to it as the default PIC upscaler and snap-blend is on its way out of that path.

Two scope caveats before the enthusiasm: omyac renders the **visible layer only**, at a fixed 320×190 → 6×. Game runtime also needs the priority and control layers (collision and depth sorting), which the snap lineage still provides. "Default upscaler" here means the PIC visual path, not the whole renderer.
...
The pixel-space approach was always going to hit a ceiling. Reasoning about commands and topology instead of pixels gets past it.
...
<VizFigure
  name="The omyac-upscaler pipeline and where the five SCI0 fixes attach"
  summary="PIC commands are replayed into a 320 by 190 native pre-render that records colour byte, command index and command type per cell; fix 1 keeps the full doubled-nibble byte there and fix 5 promotes untouched cells to white fills at its end. The anchor graph projects line anchors and connects them, with fix 2 making its sameColor check use cmdType. The hybrid render draws Bresenham strokes at 6 times scale, then the enhance kernel votes neighbourhoods, where fixes 3 and 4 widen its vote buckets to 256 and break ties with the perceptual blend table. After enhance, a final null-pixel mode fill runs and the doubled-nibble output is resolved to RGBA through the blend table."
>
...
<Figure caption="The wireframe stage on the spacecraft PIC, before any enhance pass. The hot pink is every pixel not yet written — expected here, mid-pipeline, unlike pink in final output.">
  ![Pre-enhance hybrid render: anchor strokes and dots trace the scene over a hot-pink unwritten field](./wireframe-pre-enhance.png)
...
<Figure caption="The same PIC after the default enhance sequence and the final null-pixel sweep: the pink gaps are gone and every fill region has converged to solid colour.">
  ![After the enhance passes: pink gaps replaced by solid fills, a complete scene with crisp lines](./wireframe-enhanced.png)
...
The byte carries colour and the cmdType buffer carries provenance, so the two concerns stay separated.
...
This isn't strictly required for correctness (without it the algorithm still converges), but tied regions look obviously wrong without it, and the table was already there to pull in.

### Fix 5: untouched cells mean "white background," not "gap"

![Diagnostic before fix 5: nearly all the PIC magenta, only strokes and a few regions in real colour](./null-cell-diagnostic-hot-pink.png)
...
The fix is one loop, but it's the only one of the five about what "no command plotted here" _means_ in each engine's PIC convention rather than about how SCI0 encodes colour.
...
<Figure caption="Scrapyard exterior: a toppled ACME cylinder and a cyan hull with lettering." zoom>
  <img src={sweep1.src} width="1920" height="1140" alt="Wipe comparison of a junkyard scene: a white ACME cylinder, grey paving stones and a cyan hull" />
</Figure>

<Figure caption="Scrapyard with a magenta stairway and a teal dome." zoom>
  <img src={sweep2.src} width="1920" height="1140" alt="Wipe comparison of a magenta stepped ramp and dark blue arch, with a teal dome and a purple tower" />
</Figure>

<Figure caption="Interior with two lit arches and a skeletal machine." zoom>
  <img src={sweep3.src} width="1920" height="1140" alt="Wipe comparison of a blue interior with two cyan-rimmed arches, an orange dune and machinery" />
</Figure>

<Figure caption="A red cylindrical corridor seen side-on." zoom>
  <img src={sweep4.src} width="1920" height="1140" alt="Wipe comparison of a red cylindrical corridor with slatted panels and pipes on a black background" />
</Figure>

<Figure caption="Hangar with a green-jewelled machine and a red-and-cyan gantry." zoom>
  <img src={sweep5.src} width="1920" height="1140" alt="Wipe comparison of a grey machine with a green diamond jewel, beside a cyan gantry with red panels" />
</Figure>

<Figure caption="Cavern floor with a crashed grey ship, a white pod and yellow lamp-posts." zoom>
  <img src={sweep6.src} width="1920" height="1140" alt="Wipe comparison of a crashed grey ship on a cavern floor, with a white pod and a yellow lamp-post" />
</Figure>

<Figure caption="Blue ice cavern with a green vehicle and thin outline geometry." zoom>
  <img src={sweep7.src} width="1920" height="1140" alt="Wipe comparison of a dark blue ice cavern in thin outlines, with a green vehicle at the centre" />
</Figure>
...
Diagonals are clean. Dithered regions are smooth. Background whites are white. Junctions where multiple polylines meet produce continuous geometry rather than stair-stepped block expansions. Solid-fill regions match the existing perceptual dither rendering for the same input bytes, with cleaner line geometry layered on top. On every PIC the sweep page can show, the omyac output looks like it was rendered by an engine that _understood_ the picture rather than one that was trying its best with the pixels it got. I'm committing to it as the default for the PIC visual path.
...
If that turns out to be a problem the fix is shallow: widen omyac's stroke by planting larger anchor dots, drawing thicker Bresenham strokes, or post-thickening line-type pixels in a final pass. None of those are deep algorithmic changes; they're tuning knobs against a chosen target weight. But the right value can only be picked by eyeballing the composite of PIC + views in motion, which is the eval that's still to come. If the answer is "thinner lines are fine" we ship as-is; if it's "thinner lines clash" we add a stroke-width parameter and tune until the composite reads right. Either way it isn't a blocker for the migration, just the last tuning step in front of it.

## What still doesn't work: small highlights at 6× upscale

One limitation: **isolated single-cell highlights don't fully expand at 6× scale.**
...
- **Encode colour identity out-of-band.** Keep the byte for colour and a separate buffer for command provenance, because any format where one byte means two things (solid + dither pair, white + sentinel) breeds bugs otherwise.
- **Audit paradigm conventions, not just data formats.** AGI artists cover the canvas and SCI0 artists rely on initial-state defaults, so wherever the algorithm asks what a cell means, normalise the answer at the earliest point in the pipeline (the fix-5 sweep is the example).
- **Trust the host project's perceptual machinery.** Use an existing perceptually-blended dither table for tie-breaks; nearest-RGB only works in AGI because the palette is small enough that the metric doesn't matter.
- **Pick an upscale factor the algorithm was designed for, or accept a small-feature gap.** The isolated-pixel splat assumes a cell's territory is small, and at 6× that ratio breaks, so size the limitation up before committing to a factor.
...
The port stayed shippable from the first compile. Each fix was a few lines in a single function, applied incrementally and rendered side-by-side against snap-blend in the pic-dev sweep page after every change. There was never an intermediate state where the renderer didn't render. The hot-pink diagnostic landed the same week as fix 5, and neither blocked the other or invalidated work that depended on the previous behaviour.

That worked because the algorithm's structure (provenance, anchors, topology, voting) is decoupled from its encoding (how a colour fits in a byte). Encoding fixes only touched the encoding boundary. Algorithm fidelity was never up for negotiation: the C reference passes intact, with adapter logic at the surface.

The two strategies coexisted on purpose. Both registered through the same strategy interface, consumed the same SCI0 PIC bytestream, and produced the same RGBA output shape, so the sweep page could show omyac and snap-blend on the same PICs at the same time. Adding the port was a strict superset of existing functionality; nothing was replaced or torn out while the port was in flight. The verdict on PIC rendering is in: omyac wins and becomes the default. snap-blend stays registered as the comparison through the views-against-line-weight eval, and once that and any stroke-width tuning settle, it retires from the visible-layer PIC path (game runtime keeps snap for the priority and control layers omyac doesn't render).

The tempting alternative is a big-bang rewrite: branch the renderer pipeline and merge it back when "everything works." Every one of fixes 1–5 would then have surfaced during the merge instead of during development, argued about in aggregate rather than understood one at a time. A small, registered, side-by-side strategy makes it impossible to confuse "the algorithm is broken" with "this specific encoding boundary is broken," because the artefact is visible against a known-good comparison every time.

That's the part worth keeping for future renderer work: the next strategy should also land registered, comparable, and shippable from the first commit, even if the eventual outcome is replacing what came before. Big-bang rewrites of rendering pipelines don't pay back their volatility cost; _eventual_ replacement after side-by-side validation does.
