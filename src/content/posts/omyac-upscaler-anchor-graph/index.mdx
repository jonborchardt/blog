...
flowchart TB
  ...
  accDescr: PIC commands are replayed into a 320 by 190 native pre-render that records colour byte, command index and command type per cell. Fix 1 keeps the full doubled-nibble byte there and fix 5 promotes untouched cells to white fills at its end. The anchor graph projects line anchors and connects them; fix 2 makes its sameColor check use cmdType. The hybrid render draws Bresenham strokes at 6x, then the enhance kernel votes neighbourhoods; fixes 3 and 4 widen its vote buckets to 256 and break ties with the perceptual blend table. After enhance, a final null-pixel mode fill runs and the doubled-nibble output is resolved to RGBA through the blend table.
  ...
  E --> O[Final null-pixel mode fill<br/>blend-table resolve to RGBA]
...
<Figure caption="The wireframe stage on the spacecraft PIC, before any enhance pass. The hot pink is every pixel not yet written — expected here, mid-pipeline, unlike pink in final output.">
  ![The pre-enhance hybrid render of the spacecraft PIC: thin anchor-to-anchor strokes and per-cell anchor dots trace the scene's geometry, with all unwritten pixels rendered hot pink so the image reads as line art over a pink field](./wireframe-pre-enhance.png)
...
<Figure caption="The same PIC after the default enhance sequence and the final null-pixel sweep: the pink gaps are gone and every fill region has converged to solid colour.">
  ![The spacecraft PIC after the enhance passes: the wireframe's pink gaps are fully replaced by solid fill regions, leaving a complete scene with crisp line geometry and no hot pink remaining](./wireframe-enhanced.png)
...
Diagonals are clean. Dithered regions are smooth. Background whites are white. Junctions where multiple polylines meet produce continuous geometry rather than stair-stepped block expansions. Solid-fill regions match the existing perceptual dither rendering for the same input bytes, with cleaner line geometry layered on top. The difference compared to snap-blend is not subtle — on every PIC the sweep page can show, the omyac output looks like it was rendered by an engine that _understood_ the picture rather than one that was trying its best with the pixels it got. We're committing to it as the default for the PIC visual path.
