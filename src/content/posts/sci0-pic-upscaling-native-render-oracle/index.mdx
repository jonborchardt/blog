...
Across the test corpus at 6x, snap-blend produced no catastrophic fill floods: escapes are caught mid-fill, rolled back, and recovered from the native mask. The flagged fills that remain are small boundary-thickness deviations, not leaks. The blend pass is purely cosmetic — same correctness, nicer pixels.
