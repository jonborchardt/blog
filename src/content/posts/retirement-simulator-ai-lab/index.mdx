...
The verdicts live in a running scoreboard (theories.md, in that same private repo). Three verdicts so far: the bucket strategy lost outright; "own everything + guardrails" lost to the targeted hedge; and the tax-coordination videos were the first outside advice to beat the champion, though only as a sensitivity bound, since the flat-tax engine can't verify the low rate. By the end, most new theories were adjudicated entirely from rows already run, which is what happens when new advice keeps decomposing into experiments already done. The full set of verdicts is its own post: [the simulator watches retirement YouTube](/simulator-watches-retirement-youtube/).
