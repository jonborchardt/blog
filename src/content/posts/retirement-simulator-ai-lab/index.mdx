...
hero:
  src: ./hero.png
  alt: A chart of many overlaid wealth-over-time lines, one per historical start year from 1928 onward, most rising and two sinking to zero after retirement, with a lab flask and a JSON experiment tag beside them
draft: false
---
...
Every year I sit down and answer one question: **what's the earliest age I can stop working without ever running out of money?** Instead of a spreadsheet, I have [retire-sim](https://github.com/jonborchardt/retire-sim), a small deterministic historical-replay simulator that's become something more interesting: a lab, with a lab protocol, and an AI lab assistant that follows it.

<Figure caption="An earlier season's results grid. The 'deep guardrail 10y/25%' row beating the honest guardrail is exactly the dishonest-guardrail trap described below — that row, the micro-cap tilt, and the golden-butterfly rows have since been deleted from the committed catalog (the bans are now enforced by the validator), so a fresh clone shows a smaller grid.">
  ![Results grid from retire-sim on the synthetic national-average dataset: strategies as rows, four candidate retirement ages (62, 65, 67, 70) as columns at Social Security claim age 62, each cell showing survival percentage then median and worst-case terminal wealth in today's dollars, with cells clearing the 85 percent survival bar in bold](./strategy-grid.png)
...
flowchart TB
  ...
  accDescr: Committed experiment rows and the committed 1928 to 2025 history feed a validation step together with the chosen dataset, either a gitignored personal config or a committed synthetic one; the validated inputs reach the simulator, which replays every start year at every retirement and claim age and writes survival percentage plus median and worst-case terminal wealth into one results cell per strategy and age.
  E["experiments.json<br/>one row per strategy, committed"]
  H["history 1928-2025<br/>committed"]
  C["dataset: gitignored personal config<br/>or committed synthetic one"]
  E --> V
  C --> V
  V["validate: no micro-caps, livable guardrails,<br/>no personal numbers in committed rows"] --> S
  H --> S
  S["replay every start year<br/>x retire age x claim age"] --> R["cell: survival %,<br/>median and worst-case terminal wealth"]
```
...
<Figure caption="The wealth fan chart on the synthetic dataset. Screenshot from an earlier dataset revision with a longer horizon; the committed dataset has since shortened the horizon and added a two-person survivor model, so a fresh run ends in 2067.">
  ![Fan chart of household wealth in today's dollars from 2026 to 2071: 98 faint paths, one per historical start year from 1928 to 2025, with a shaded 10th to 90th percentile band and a median line, and a hover readout for 2032 showing 10th to 90th percentile 117,332 to 329,437 and median 231,387](./wealth-fan-chart.png)
...
flowchart TB
  ...
  accDescr: A source URL is required and its captions are fetched; the transcript is sorted into testable mechanisms, assumptions fact-checked against history, and framing mapped to a real mechanism; the mechanisms become three experiment rows, faithful, milder dose and hybrid on the champion, which the simulator runs against every start year and compares with the baseline and champion; the failing start years are diagnosed by cohort, and the verdict fans out to the theories scoreboard, the journal and the scored sources list.
  U["source URL required<br/>captions via npm run transcript"] --> T[transcript or article]
  T --> S[sort claims]
  S --> M[testable mechanisms]
  S --> A["assumptions<br/>fact-checked vs history"]
  S --> V["framing<br/>mapped to a real mechanism"]
  M --> F[row 1: faithful]
  M --> Hd[row 2: milder dose]
  M --> G[row 3: hybrid on champion]
  V --> M
  A --> D
  F --> R[run vs baseline and champion]
  Hd --> R
  G --> R
  R --> Co["cohort diagnosis<br/>which start years failed"]
  Co --> D[verdict]
  D --> TH[row in theories.md]
  D --> J["journal: notes plus dated learnings"]
  D --> SR[score in sources.json]
```
...
None of this tells you to buy gold. It tells you the method: find which historical regimes break _your_ plan, test remedies against those exact years, distrust anything your instruments can't reproduce, know your noise floor, and write down what you learned next to the data you learned it on. In the final rounds even the sim's own flattering assumptions got that treatment: each caveat — ignored taxes, rich equity valuations, gold mean-reversion, the joint model — was made runnable as a permanent return drag and priced in working years (roughly one to two each), and the plan's edge over the baseline _widens_ under identical pessimism. The honest insurance against unmodeled tails turned out to be one more working year, not a different portfolio. And it turns out this whole method is exactly the kind of thing you can teach an AI to run.
